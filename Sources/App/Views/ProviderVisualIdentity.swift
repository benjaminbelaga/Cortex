...
enum ProviderVisualIdentityLookup {
    ...
    /// Short display abbreviation for a runtime model id (e.g. "MiniMax-M3"
    /// → "M3"). Returns nil when the id is unknown — caller falls back to a
    /// harness-only label. SSOT lives in
    /// `~/repos/llm-router/config/providers.yaml#display_abbreviations` and is
    /// rendered at build time into `ModelAbbreviation.generated.json`.
    static func abbreviation(for modelId: String?) -> String? {
        ModelAbbreviation.abbreviation(for: modelId)
    }
