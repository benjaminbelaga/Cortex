    ...
    var body: some View {
        SettingsPane(
            ...
            VStack(spacing: 14) {
                ...
                VStack(spacing: 4) {
                    Text("Git \(provenance.shortSHA) · \(provenance.isDirty ? "dirty" : "clean")")
                        .font(.system(size: 10, weight: .semibold, design: .monospaced))
                        .foregroundStyle(provenance.isDirty ? theme.statusWarning : theme.statusHealthy)
                    Text(provenance.builtAtUTC)
                        .font(.system(size: 9, design: .monospaced))
                        .foregroundStyle(theme.textTertiary)
                    Text(provenance.gitSHA)
                ...
                Link(destination: URL(string: "https://github.com/benjaminbelaga/Cortex")!) {
                    HStack(spacing: 6) {
