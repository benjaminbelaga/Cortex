...
/// Status-line adapter kill-switch (D2 tranche, opt-in).
/// Read through this narrow protocol so Domain never depends on the concrete
/// JSON store. Tests provide a stub; production conformance lives on
/// `JSONSettingsRepository` next to the other Claude settings.
public protocol StatusLineAdapterSettings {
    /// Whether the passive Claude status-line adapter is enabled.
    func isClaudeStatusLineAdapterEnabled() -> Bool
}
