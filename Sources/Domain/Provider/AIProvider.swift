...
/// Distinguishes a manual refresh, a popover-open refresh, and the background poll.
///
...
public enum RefreshKind: Sendable, Equatable {
    /// Explicit button/account/provider action: force every local usage source.
    case interactive
    /// Opening the popover: quota is refreshed, usage may reuse a <=60s cache.
    case opening
    /// Periodic/wake refresh: cheap provider-specific work, bounded usage cache.
    case background

    /// Higher-priority requests supersede an in-flight lower-priority overview pass.
