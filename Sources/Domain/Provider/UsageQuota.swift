...
public struct UsageQuota: Sendable, Equatable, Hashable, Comparable {
    ...
    /// True when the reading behind this quota is too old / low-confidence to
    /// count as a live signal (e.g. a multi-day-old manual Qwen sync, an
    /// errored provider's phantom 0%). Stale quotas still render in their row
    /// (muted), but never drive the menu-bar health glyph or the "under 10%"
    /// alarm — a stale 0% is not a real exhaustion (Ben 2026-08-24).
