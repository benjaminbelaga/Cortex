...
/// Installs and uninstalls Cortex hooks in ~/.claude/settings.json.
/// Hook commands use the __claudebar_hook function wrapper for identification.
public enum HookInstaller {
    /// The marker function name used to identify Cortex hooks
    ...
    public static func install() throws {
        ...
        for event in hookEvents {
            ...
            // Remove any existing Cortex matcher entries for this event
            matcherEntries.removeAll { entry in
                containsCortexHook(in: entry)
            }
    ...
    /// Uninstalls Cortex hooks from the Claude settings file.
    ...
    /// Detects whether Cortex hooks are currently installed.
    ...
    /// Checks if a matcher entry contains a Cortex hook command.
    private static func containsCortexHook(in matcherEntry: [String: Any]) -> Bool {
