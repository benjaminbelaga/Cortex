...
public struct OpenCodeAPIUsageProbe: UsageProbe, @unchecked Sendable {
    ...
    public func isAvailable() async -> Bool {
        if explicitAPIKey != nil || credentialLoader.loadAPIKey() != nil {
            return true
        }
        if let fallback {
    ...
    public func probe() async throws -> UsageSnapshot {
        let pool = explicitAPIKey.map { [OpenCodePoolEntry(slot: "managed", label: nil, key: $0)] } ?? credentialLoader.loadPool()
        ...
        // Multi-account pool (failover SSOT): one quota section per account,
        // grouped like Oh My Pi upstream accounts. A dead account is skipped
        // (logged); all-dead throws like the single-key path.
        let grouped = pool.count > 1
        var quotas: [UsageQuota] = []
        var lastError: Error?
        for entry in pool {
            do {
                let data = try await fetchUsage(apiKey: entry.key)
                let group = grouped ? "OpenCode Go · \(entry.label ?? entry.slot)" : nil
                quotas += try Self.parseResponse(data, group: group).quotas
            } catch {
                AppLog.probes.error("OpenCode: account \(entry.label ?? entry.slot) failed: \(error.localizedDescription)")
                lastError = error
            }
        }
        guard !quotas.isEmpty else {
            throw lastError ?? ProbeError.authenticationRequired
        }

        let summary = quotas
            .map { "\($0.group.map { "\($0) " } ?? "")\($0.quotaType.displayName) \(Int($0.percentRemaining))%" }
            .joined(separator: ", ")
        AppLog.probes.info("OpenCode API probe success: \(summary)")

        return UsageSnapshot(providerId: Self.providerId, quotas: quotas, capturedAt: Date())
    }
    ...
    static func parseResponse(_ data: Data, group: String? = nil, now: Date = Date()) throws -> UsageSnapshot {
