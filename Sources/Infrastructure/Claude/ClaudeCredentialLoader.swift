...
public struct ClaudeCredentialLoader: Sendable {
    ...
    ///
    /// A payload written by an older Cortex build comes back hex-encoded;
    ...
    public init(
        ...
        self.keychainService = keychainService == "Claude Code-credentials"
            ? ClaudeProfileLocation.keychainService(directory: configDirectory ?? environment["CLAUDE_CONFIG_DIR"], home: homeDirectory)
            : keychainService
    ...
    public var credentialsFilePath: String {
        if let custom = ClaudeProfileLocation.customDirectory(resolvedConfigDirectory, home: homeDirectory) {
            return (custom as NSString).appendingPathComponent(".credentials.json")
        }
    ...
    public func loadCredentials() -> ClaudeCredentialResult? {
        ...
        if ClaudeProfileLocation.customDirectory(configDirectory ?? environment["CLAUDE_CONFIG_DIR"], home: homeDirectory) == nil,
           let envResult = loadFromEnvironment() {
            return envResult
        }
