...
- **The token is sensitive.** It goes to the Keychain-backed store under `CredentialKey.notifyDeviceToken` and never to `~/.claudebar/settings.json`. It is never logged, and neither is the device id above debug level, which never reaches the log file on disk.
- **When the Keychain refuses, the token falls back.** A locally built Cortex is ad-hoc signed (`CODE_SIGN_IDENTITY` is `-`), so macOS cannot attach a stable app identity and returns `errSecAuthFailed` (-25293). A release build signed with a Developer ID is unaffected. The save is read back and, when it did not stick, the token uses the existing local settings store. It is still never written to `settings.json`; the pane reports which store won.
