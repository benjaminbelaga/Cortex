...
Cortex already knows every quota worth knowing, and the phone is where the user actually looks. [Notify!](https://getnotifyapp.com) already exists, runs on Mac and iOS and reaches anything else through web push, and has a documented gateway API, so Cortex can put a quota on a Lock Screen and a Home Screen without shipping an iOS app of its own.

---

## The problem

Every surface Cortex has is on the Mac, and every one of them needs the Mac awake and in front of you.
...
- **Percentages are remaining, not used.** A 42% quota publishes `progress: 42`. Every other Cortex surface reads that way, and a full ring meaning a full quota is the only intuitive mapping a gauge has.
...
| What happened | HTTP | What Cortex does |
|---|---|---|
...
Some failures are not worth discovering over the wire. A Live Activity aimed at a `GRP`, `MC` or `WB` id is refused locally, before a request exists, because the outcome is already known: the gateway answers that start with a 400 saying the device cannot show one. Either widget aimed at a `GRP` id is refused for a different reason, that a group is not a device and owns no widget list to write into. What the user reads in both cases is the id's own reason, which names the kind of device they linked and tells them to use the device ID and token from their phone, rather than a status code that reads as Cortex failing at something it should have managed.
...
Every other network call Cortex makes fetches a quota from the provider that owns it. This one is the first that sends Cortex's own state outward, to a service that is neither the user's machine nor a provider they already have an account with, so it is worth being explicit.
...
Note which way round the Live Activity rule is written. The namespaces that cannot show one are named and everything else is allowed, rather than the reverse, because app device ids are not one fixed shape: the legacy format is 8 characters, `IO` is 16, and more will follow. A list of what may pass would refuse a real phone on the day Notify! mints a format Cortex has never heard of, and a refused phone reads as Cortex being broken rather than as a new namespace.

Cortex drives no notification route, so that column is not something it uses; it is there because it is what makes such an id a real id rather than a typo, and so a thing to explain rather than to refuse.
...
Both widgets are the opposite, and identically so. The gateway is explicit that neither route carries a device type gate: legacy, `IO`, `WB` and `MC` ids can all own a Lock Screen widget, and it says the same of Home Screen widgets in so many words. Which of them actually draws one is Notify!'s business rather than a rule for Cortex to invent. Only a group is refused, and only because a group is not a device: it has members, and no widget list of its own for a gauge or a tile to sit in. `NotifyDeviceKind.supportsScreenWidget` is therefore written as `supportsWidget` rather than as a second copy of the same switch, so the two can never drift into disagreeing about one id.
...
- **503** is the server side kill switch, and no other route has one. Home Screen widgets ship behind it, so creates and updates can be refused while reads and deletes stay deliberately open and an already placed tile keeps rendering. It means the feature is not being served yet, not that anything is wrong with Cortex, the credentials or the content, so it becomes `surfaceSwitchedOff`, which is retryable and whose sentence says Cortex will try again later.
- **409** here means the device dialect found several screen widgets and cannot say which was meant. The shared mapping reads a 409 as `liveActivityUnavailable`, because that is what it means on the route it was written for, and sending somebody to open the Notify! app about their Live Activities would be a wrong answer to a question about a Home Screen widget. Cortex creates its own with `new` and addresses it by `SW…` id afterwards, so it should never meet this at all.

### Two dialects, and why Cortex only uses one of them
...
| Key | Holds |
|---|---|
| `notify.enabled` | Whether Cortex publishes at all. Default `false`. |
...
| `notify.activityId`, `notify.widgetId`, `notify.screenWidgetId` | The handles of the Live Activity, the Lock Screen widget and the Home Screen widget Cortex created. |
...
- a 503 on the Home Screen route is a switched-off surface rather than a failure: retryable, and carrying its own "Cortex will try again later" sentence when the gateway named none
...
- every field Cortex drives is present in the body on every write, as an explicit null when it has no value, while the fields it never drives stay unmentioned
...
- **More than one gauge.** The gateway allows 10 widgets per device, so a widget per provider is possible. Unresolved: the widget title is its identity in the phone's picker, so several widgets need distinguishable titles that are still recognizably Cortex, and it is not obvious whether four rings is more useful than one ring plus the metrics tile already beside it.
