...
struct PopoverContentHeightTests {
    ...
    func `overview shrinks below its ceiling on short displays`() {
        // 600pt visible frame → 500 + chrome would overflow; the remainder wins.
        let cap = PopoverContentHeight.maxHeight(visibleScreenHeight: 600, overviewMode: true)
        #expect(cap == 600 - PopoverContentHeight.chrome)
    }
