...
struct QuotaMonitorTests {
    ...
    private final class CountingUsageProbe: UsageProbe, @unchecked Sendable {
        ...
        init(providerId: String, delay: Duration? = nil) {
    ...
    func `menu bar label carries both windows as separate segments`() async {
        ...
        #expect(label?.status == .warning)
        #expect(label?.segments == [
            MenuBarLabel.Segment(text: "5h 75%", status: .healthy, percentRemaining: 75),
            MenuBarLabel.Segment(text: "7d 35%", status: .warning, percentRemaining: 35),
        ])
    }
    ...
    func `menu bar label prefers the quota's menuBarTitle for window prefixes`() async {
        ...
        #expect(label?.segments == [
            MenuBarLabel.Segment(text: "Claude 7d · jkjk987… 69%", status: .healthy, percentRemaining: 69),
            MenuBarLabel.Segment(text: "7d 35%", status: .warning, percentRemaining: 35),
        ])
    }
    ...
    func `menu bar label segments cover percentage plus duration windows`() async {
        ...
        #expect(label?.status == .warning)
        #expect(label?.segments == [
            MenuBarLabel.Segment(text: "5h 75% · 3:58", status: .healthy, percentRemaining: 75),
            MenuBarLabel.Segment(text: "7d 35% · 6d", status: .warning, percentRemaining: 35),
        ])
    }
    ...
    @Test
    func `overlapping overview opens coalesce to one refresh`() async throws {
        let settings = makeSettingsRepository()
        let probe = CountingUsageProbe(providerId: "claude", delay: .milliseconds(100))
        let provider = ClaudeProvider(probe: probe, settingsRepository: settings)
        let monitor = makeMonitor(providers: AIProviders(providers: [provider]))

        async let first: Void = monitor.refreshOverview(kind: .opening)
        try await Task.sleep(for: .milliseconds(10))
        async let second: Void = monitor.refreshOverview(kind: .opening)
    ...
    @Test
    func `manual overview request queues once behind background refresh`() async throws {
        let settings = makeSettingsRepository()
        let probe = CountingUsageProbe(providerId: "claude", delay: .milliseconds(100))
        let provider = ClaudeProvider(probe: probe, settingsRepository: settings)
        let monitor = makeMonitor(providers: AIProviders(providers: [provider]))

        async let background: Void = monitor.refreshOverview(kind: .background)
        try await Task.sleep(for: .milliseconds(10))
        async let manual: Void = monitor.refreshOverview(kind: .interactive)
        _ = await (background, manual)
    ...
    // MARK: - Follow / Unfollow / Replace (activation à la demande)

    @Test
    func `follow instantiates a missing provider through the factory and starts it`() {
        // Given — a monitor whose repository has no codex instance yet.
        // The mock is fully stubbed: follow() fires a fire-and-forget refresh
        // that may outlive the test, and an unstubbed mock would report the
        // missing stub into a freed test context (use-after-free crash).
        let settings = makeSettingsRepository()
        let probe = MockUsageProbe()
        given(probe).isAvailable().willReturn(false)
        ...
        let monitor = QuotaMonitor(
            ...
            clock: TestClock(),
            providerFactory: { id in id == "codex" ? codex : nil }
        )

        // When
        let followed = monitor.follow(providerId: "codex")
    ...
    @Test
    func `follow is idempotent — never two instances for one id`() {
        // Given — same stubbing discipline as above: the follow() refresh
        // task must never touch an unstubbed mock after teardown.
        let settings = makeSettingsRepository()
        let probe = MockUsageProbe()
        given(probe).isAvailable().willReturn(false)
        let codex = CodexProvider(probe: probe, settingsRepository: settings)
        let monitor = QuotaMonitor(
            providers: AIProviders(providers: []),
            clock: TestClock(),
            providerFactory: { _ in codex }
        )

        // When
        _ = monitor.follow(providerId: "codex")
        _ = monitor.follow(providerId: "codex")
    ...
    @Test
    func `follow returns false when nothing can be built for that id`() {
        // Given — no factory at all (an install without a catalogue).
        let settings = makeSettingsRepository()
        let claude = ClaudeProvider(probe: MockUsageProbe(), settingsRepository: settings)
        let monitor = makeMonitor(providers: AIProviders(providers: [claude]))

        // When / Then
        #expect(monitor.follow(providerId: "unknown-tool") == false)
        #expect(monitor.allProviders.count == 1)
    }

    @Test
    func `unfollow stops collecting but keeps the instance and its last reading`() async throws {
        // Given — a provider that has already produced one reading.
        let settings = makeSettingsRepository()
        let probe = CountingUsageProbe(providerId: "codex")
        let codex = CodexProvider(probe: probe, settingsRepository: settings)
        let monitor = makeMonitor(providers: AIProviders(providers: [codex]))

        _ = try await monitor.refresh(providerId: "codex")
        #expect(codex.snapshot != nil)

        // When
        monitor.unfollow(providerId: "codex")

        // Then — rétention : la ligne et son relevé restent, la collecte s'arrête.
        #expect(monitor.provider(for: "codex") != nil)
        #expect(codex.snapshot != nil)
        #expect(monitor.enabledProviders.isEmpty)
    }

    @Test
    func `replaceProvider swaps the instance without leaving two collectors`() {
        // Given
        let settings = makeSettingsRepository()
        let first = CodexProvider(probe: MockUsageProbe(), settingsRepository: settings)
        let monitor = makeMonitor(providers: AIProviders(providers: [first]))

        // When — changement de source : nouvelle instance pour le même id.
        let second = CodexProvider(probe: CountingUsageProbe(providerId: "codex"), settingsRepository: settings)
        monitor.replaceProvider(second)

        // Then
        #expect(monitor.allProviders.count == 1)
        #expect((monitor.provider(for: "codex") as AnyObject?) === second)
        #expect(monitor.enabledProviders.map(\.id) == ["codex"])
    }
