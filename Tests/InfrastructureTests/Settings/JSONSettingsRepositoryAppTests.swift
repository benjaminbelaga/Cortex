...
struct JSONSettingsRepositoryAppTests {
    ...
    @Test
    func `backgroundSyncEnabled defaults to false (upstream public default)`() {
        let (repo, dir) = makeRepository()
        defer { cleanup(dir) }
    ...
    @Test
    func `explicitly disabled background sync remains off`() {
        let (repo, dir) = makeRepository()
        defer { cleanup(dir) }

        repo.setBackgroundSyncEnabled(false)
        #expect(repo.backgroundSyncEnabled() == false)
    }

    @Test
    func `backgroundSyncInterval defaults to 600 (upstream power-conscious default)`() {
        let (repo, dir) = makeRepository()
        defer { cleanup(dir) }

        // Fresh operational default; sleep and battery policies remain active.
