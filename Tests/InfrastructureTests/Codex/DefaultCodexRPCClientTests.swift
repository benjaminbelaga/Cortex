...
struct DefaultCodexRPCClientTests {
    ...
    @Test func readsCurrentPoolsAndPreservesWindowDuration() throws {
        let client = DefaultCodexRPCClient()
        let result: [String: Any] = ["rateLimits": ["primary": ["usedPercent": 99]],
            "rateLimitsByLimitId": [
                "codex": ["primary": ["usedPercent": 24, "windowDurationMins": 300, "resetsAt": 1800000000],
                          "secondary": ["usedPercent": 50, "windowDurationMins": 10080]],
                "spark": ["limitName": "Spark", "primary": ["usedPercent": 10, "windowDurationMins": 300]]
            ]]
        let limits = try client.parseRateLimits(result)
        #expect(limits.primary?.usedPercent == 24)
        #expect(limits.primary?.windowDurationMins == 300)
        #expect(limits.primary?.resetsAt == Date(timeIntervalSince1970: 1800000000))
        #expect(limits.additionalPools.count == 1)
        let snapshot = try CodexUsageProbe.mapRateLimitsToSnapshot(limits)
        #expect(snapshot.quotas.count == 3)
        #expect(snapshot.quotas.last?.group == "Spark")
    }

    @Test func weeklyOnlyPrimaryDoesNotBecomeSession() throws {
        let client = DefaultCodexRPCClient()
        let limits = try client.parseRateLimits(["rateLimits": ["primary": ["usedPercent": 15, "windowDurationMins": 10080]]])
        let snapshot = try CodexUsageProbe.mapRateLimitsToSnapshot(limits)
    ...
    @Test
    func `fetchRateLimits preserves missing free plan windows`() async throws {
        // Given
        let mockTransport = MockRPCTransport()
        let mockExecutor = MockCLIExecutor()
        setupMockTransport(mockTransport, rateLimitsResponse: """
    ...
    @Test func missingWindowsRemainUnknownWithoutStartingTTY() async throws {
        let transport = MockRPCTransport()
        setupMockTransport(transport, rateLimitsResponse: #"{"id":2,"result":{"rateLimits":{"planType":"pro"}}}"#)
        let limits = try await DefaultCodexRPCClient(transport: transport).fetchRateLimits()
        #expect(limits.primary == nil)
        #expect(throws: ProbeError.self) { try CodexUsageProbe.mapRateLimitsToSnapshot(limits) }
    }
