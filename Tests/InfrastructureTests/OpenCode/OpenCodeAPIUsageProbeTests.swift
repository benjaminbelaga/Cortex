...
struct OpenCodeAPIUsageProbeTests {
    ...
    private func poolLoader() throws -> OpenCodeCredentialLoader {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("opencode-pool-tests-\(UUID().uuidString)", isDirectory: true)
        let authDir = home.appendingPathComponent(".local/share/opencode", isDirectory: true)
        try FileManager.default.createDirectory(at: authDir, withIntermediateDirectories: true)
        let auth: [String: Any] = [
            "opencode-go": ["type": "api", "key": "key-A"],
            "opencode": ["type": "api", "key": "key-B"],
        ]
        try JSONSerialization.data(withJSONObject: auth).write(to: authDir.appendingPathComponent("auth.json"))
        return OpenCodeCredentialLoader(homeDirectory: home.path, environment: [:])
    }

    @Test
    func `probe groups quotas per pool account`() async throws {
        let network = MockNetworkClient()
        given(network).request(.any).willReturn((OpenCodeAPIUsageProbeParsingTests.usageJSON, httpResponse(200)))

        let probe = OpenCodeAPIUsageProbe(credentialLoader: try poolLoader(), networkClient: network)
        let snapshot = try await probe.probe()

        #expect(snapshot.quotas.count == 6)
        #expect(snapshot.quotaGroups.map(\.title) == ["OpenCode Go · compte 1", "OpenCode Go · compte 2"])
    }

    @Test
    func `probe skips dead account and keeps the live one`() async throws {
        let network = MockNetworkClient()
        given(network).request(.any).willProduce { request in
            if request.value(forHTTPHeaderField: "Authorization") == "Bearer key-A" {
                return (Self.errorJSON, self.httpResponse(401))
            }
            return (OpenCodeAPIUsageProbeParsingTests.usageJSON, self.httpResponse(200))
        }
        ...
        #expect(snapshot.quotas.count == 3)
        #expect(snapshot.quotaGroups.map(\.title) == ["OpenCode Go · compte 2"])
    }

    @Test
    func `probe throws when every pool account fails`() async throws {
        let network = MockNetworkClient()
        given(network).request(.any).willReturn((Self.errorJSON, httpResponse(401)))
