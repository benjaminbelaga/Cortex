...
struct BinaryLocatorTests {
    ...
    func `findInCommonPaths finds executable in homebrew bin`() {
        ...
        // The assertion is intentionally order-agnostic: user-local shims
        // (e.g. ~/bin/brew) legitimately shadow system paths per commonPaths order.
        if let path {
            #expect(path.hasSuffix("/brew"))
